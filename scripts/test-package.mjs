...
try {
  ...
  const allowed = /^(?:package\.json|README\.md|CHANGELOG\.md|LICENSE|THIRD_PARTY_NOTICES\.md|docs\/[a-z]+\.md|examples\/[a-z]+\.mjs|dist\/[a-z]+\.(?:js|d\.ts)|bin\/(?:win32|darwin|linux)-(?:x64|arm64)\/(?:playsound(?:\.exe)?|manifest\.json))$/;
...
const paused = play('unused.wav', { signal: cancelled });
paused.pause(); paused.resume();
assert.equal(await paused.getTiming(), null);
...
`;
  ...
  writeFileSync(join(consumer, 'consumer.mts'), `import { play, sound, Player, AudioError, type Playback, type PlaybackResult, type PlaybackTiming } from 'node-playsound';
...
void seekResult;
const pauseResult: void = playback.pause();
